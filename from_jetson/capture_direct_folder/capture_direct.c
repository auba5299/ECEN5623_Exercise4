...
static void process_image(const void *p, int size)
{
    ...
    if(fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV)
    {
        ...
        //no processing in this direct code
        ...
        if(framecnt > -1)
        {
            ...
            if (size > sizeof(dest_buf)) errno_exit("frame too large");   //guard memcopy
            memcpy(dest_buf, p, size);
...
int main(int argc, char **argv)
{
    ...
    open_device();
    init_device();
    openlog("capture_direct", LOG_PID | LOG_CONS, LOG_USER);
    start_capturing();
    ...
    syslog(LOG_INFO, "Capture direct complete with no conversion: stored_samples=%d warmup=%d", g_frame_idx, warmup_frames);
    ...
    save_timings_csv("timings_direct.csv", acq_times_ms, proc_times_ms, write_times_ms, frame_times_ms, warmup_frames, (g_frame_idx-1));
